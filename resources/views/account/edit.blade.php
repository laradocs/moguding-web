                ...
                <div class="col-lg-8">
                    <form action="{{ route('accounts.update', $account) }}" method="post" novalidate="novalidate">
                        ...
                        <div class="form-group">
                            ...
                            <select id="device" class="custom-select @error ( 'device' ) is-invalid @enderror"
                                    name="device">
                                @foreach ( [ 'android' => '安卓', 'ios' => '苹果' ] as $key => $value )
                                    <option
                                        value="{{ $key }}" @selected(old('device', $account->device) == $key)>{{ $value }}</option>
                                @endforeach
                            </select>
                            @error ( 'device' )
                        ...
                        <div class="form-group">
                            ...
                            <input id="password" class="form-control @error ( 'password' ) is-invalid @enderror"
                                   type="password" name="password" placeholder="请输入密码" required="required" value="{{ $account->password }}"/>
                            @error ( 'password' )
                            <span class="invalid-feedback">
                                    <strong>{{ $message }}</strong>
                                </span>
