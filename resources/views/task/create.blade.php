                ...
                <div class="col-lg-8">
                    <form action="{{ route('tasks.store') }}" method="post" novalidate="novalidate">
                        ...
                        <div class="form-group">
                            ...
                            <select id="address" class="custom-select @error ('address') is-invalid @enderror" name="address">
                                @foreach ($addresses as $address)
                                    <option value="{{ $address->id }}" @selected(old('address') == $address->id)>{{ $address->full_address }}</option>
                                @endforeach
                            </select>
                            @error ('address')
                            <span class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <select id="run_role" class="custom-select" name="run_role">
                                @foreach ([ 'daily' => '每天' ] as $key => $value)
                                    <option value="{{ $key }}" @selected(old('run_rule') == $key)>{{ $value }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="input-group clockpicker">
                                <input id="run_time" class="form-control @error ('run_time') is-invalid @enderror"
                                       type="text" name="run_time" placeholder="请选择打卡时间" value="{{ old('run_time', '09:00') }}"/>
                            ...
                            @foreach ([1 => '启用', 0 => '禁用'] as $key => $value)
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input id="{{ $key }}" class="custom-control-input" type="radio" name="status"
                                           @checked(old('status', 1) == $key ? true : false) value="{{ $key }}" />
